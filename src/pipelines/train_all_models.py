"""
compare_models.py에서 사용되는 모든 모델을 학습하고 직관적인 이름으로 저장합니다.
...
def train_all_models():
    ...
    # 너무 큰 경우 서브샘플링 (빠른 학습을 위해 선택사항)
    if len(df) > 50000:
        print(f"빠른 학습을 위해 50,000개로 서브샘플링 중...")
    ...
    if os.path.exists(FEATURE_PATH):
        ...
        print(f"{len(feature_cols)}개의 선택된 특징 사용")
    else:
        feature_cols = [
            'word_count', 'sentence_count', 'avg_sent_len', 'burstiness',
            'entropy', 'ttr', 'readability', 'sentiment', 'subjectivity', 'punct_count'
        ]
        print(f"기본 {len(feature_cols)}개 특징 사용")
    ...
    acc = accuracy_score(y_test, tfidf_lr.predict(X_test_tfidf))
    print(f"  정확도: {acc:.4f}")
    ...
    acc = accuracy_score(y_test, feature_rf.predict(X_test_feat))
    print(f"  정확도: {acc:.4f}")
    ...
    acc = accuracy_score(y_test, feature_lr.predict(X_test_feat))
    print(f"  정확도: {acc:.4f}")
    ...
    acc = accuracy_score(y_test, feature_svm.predict(X_test_feat))
    print(f"  정확도: {acc:.4f}")
    ...
    acc = accuracy_score(y_test, feature_ensemble.predict(X_test_feat))
    print(f"  정확도: {acc:.4f}")
